...
const BlogReview = ({ id, ratingAcceptance }: { id: string; ratingAcceptance?: boolean }) => {
        ...
        if (mode === 0) {
            ...
            if (response.status) {
                setRatingData((prevState) => {
                    return {
                        ...prevState,
                        rates: [(response.data as CreateTourRatingResponse).rateAdded, ...prevState?.rates],
                    };
                });
                clearNewRatingData();
            }
            ...
            setRatingData((prevState) => {
                return {
                    ...
                    rates: prevState.rates.filter(
                        (rate) => rate.id !== (response.data as DeleteTourRatingResponse).deletedId
                    ),
                };
            });
        }
    ...
    return (
        <div className="w-100 my-5">
                        ...
                        <div className="d-flex justify-content-end w-100 mt-3">
                            <div className="d-flex gap-2">
                                <Button
                                    ...
                                    disabled={
                                        mode === 0
                                            ? newRating.comment.trim() === ''
                                            : editingRating.comment.trim() === ''
                                    }
                                    variant="outlined"
                                    onClick={() => {
                                        mode === 1 && setMode(0);
            ...
            <List>
                {ratingData
                    ? ratingData?.rates?.map((rating) => {
                          return (
                              <ListItem alignItems="center" className="px-0">
                                  <ListItemAvatar>
                                      <Avatar />
                                  </ListItemAvatar>
                                  <ListItemText
                                                      ...
                                                      {(popupState) => (
                                                          <>
                                                              ...
                                                              <Menu {...bindMenu(popupState)}>
                                                                  ...
                                                                  <MenuItem
                                                                      onClick={() => handleRemoveComment(rating.id)}>
