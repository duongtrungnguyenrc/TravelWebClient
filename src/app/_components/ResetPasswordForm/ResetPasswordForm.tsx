...
const ResetPassword = () => {
    ...
    const token: string | null = params.get("token") ? params.get("token")! : null;
    ...
    const handleSuccesss = (message: string) => {
    ...
    const handleSubmit : FormEventHandler<HTMLFormElement> = async (e : React.FormEvent<HTMLFormElement>) => {
        ...
        if(formData.password !== formData.confirmPassword) {
            toast.error("Confirm password must be match with your password!", {
                position: toast.POSITION.BOTTOM_RIGHT,
            });
        }
        else {
            setIsLoading(true);
            const response = await authServices.restorePassword(formData.password, token);
            setIsLoading(false);
            response.status ? handleSuccesss(response.message)
             : 
    ...
    <div className="register-site d-flex flex-column justify-content-center">
        <div className="register-site-header">
          ...
          <div>
              ...
              Vui lòng nhập và xác nhận mật khẩu để khôi phục tài khoản
              </div>
          </div>
        </div>
