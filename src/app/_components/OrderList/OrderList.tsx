    ...
    return (
        <div className="order-list-container row px-3">
                ...
                <div className="d-flex left-site">
                    ...
                    <div className="col-9 order-list-wrapper">
                                        ...
                                        orderData?.orders.map((order, index) => {
                                            return (
                                                <tr className="data-row">
                                                    <td><Radio color="info" value={index} checked={selectedItem === index} onChange={(() => setSelectedItem(index))}/></td>
                                                    <td>{ order.id } </td>
                                                    <td>{ order.tour.name }</td>
                                                    <td><div className="d-flex align-items-center gap-1"><Avatar/>{ order.contactInfo.customerFullName }</div></td>
                                                    <td><span className={"status " + (order.status.toLowerCase() === "đã thanh toán" ? "complete" : "pending")}>{ order.status }</span></td>
                                                </tr>
                                            )
                                        })
                                    }
                        ...
                        <div className="flex-center mt-2">
                            <Pagination count={orderData?.pages} onChange={(_e, page) => {page >= 0 && setPage(page)}}/>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="col-12 flex-center py-3 gap-2">
                ...
                <Button variant="outlined" onClick={() => setOpenStatistic(true)}>Xem thống kê chi tiết</Button>
                <Button variant="outlined">Hủy</Button>
            </div>
        </div>
    );
