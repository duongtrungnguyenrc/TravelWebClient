...
import { AllBlogsResponse, Blog } from '@/app/_types';
import Link from 'next/link';
import { useEffect, useState } from 'react';

const BlogsPage = ({ searchParams }: { searchParams: { page: string } }) => {
    const [lastedPosts, setLastedPosts] = useState<Blog[]>([]);
    const [data, setData] = useState<AllBlogsResponse | null>(null);

    useEffect(() => {
        const fetchData = async () => {
            const response = await blogServices.get(+searchParams?.page || 1, 10);
            const lastedResponse = await blogServices.getLastest();
            setData(response.data as AllBlogsResponse);
            setLastedPosts(lastedResponse.data as Blog[]);
        };
                        ...
                        <Grid item className="col-12 col-md-12 col-lg-8" paddingRight="3rem">
                            <Stack direction="column" gap={4}>
                                ...
                                <Grid container spacing={3}>
                                    {lastedPosts.slice(0, 2).map((post, index) => {
                                        return (
                                            <Grid key={post.id + post.author} item xs={12} lg={6}>
                                                    ...
                                                    <Card style={{ boxShadow: 'none', background: 'transparent' }}>
                                                        ...
                                                        <CardContent style={{ padding: '1rem 0' }}>
                                                            <Typography
                                                                ...
                                                                {post.time.split(' ')[0]} |{' '}
                                                                <AccessAlarmIcon sx={{ fontSize: '12px' }} />{' '}
                                                                {post.time.split(' ')[1]}
                                                            </Typography>
                        ...
                        <Grid item className="col-12 col-md-12 col-lg-4">
                            <Stack direction="column" gap={4}>
                                ...
                                <Grid container spacing={2} className="mt-3">
                                    {lastedPosts.slice(2, 4).map((post) => {
                                        return (
                                    ...
                                    <Card component="a" href="/blog/post?id=" style={{ boxShadow: 'none' }}>
                                        ...
                                        <CardContent style={{ padding: '1rem 0' }}>
                                            <Typography
                                                ...
                                                alignItems="center"
                                                gap="5px">
                                                Nguyen | <CalendarTodayIcon sx={{ fontSize: '12px' }} /> 02 December
                                                2023 | <AccessAlarmIcon sx={{ fontSize: '12px' }} /> 3 Min, to Read
                                            </Typography>
                                            <Typography variant="body1" color="text.secondary">
